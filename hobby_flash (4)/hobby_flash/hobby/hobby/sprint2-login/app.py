...
def home():
    ...
    return render_template("home.html", user=user, uid=uid)
...
def login_done():
    ...
    if DB.login(uid, pwd):
        session.clear()                  # 이전 세션 흔적 제거
        session["uid"] = uid             # ★ 반드시 다시 넣기
        user_data = DB.get_user(uid)     # DB에서 유저 정보
        session["user"] = user_data or {}  # 선택: 캐시용
        return redirect(url_for("home"))
    else:
...
def logout():
    ...
    resp.delete_cookie("session", path="/")  # ← 이름을 문자열로
    return resp
